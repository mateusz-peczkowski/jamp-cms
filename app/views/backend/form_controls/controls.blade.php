        ...
        <div class="table-responsive">
            <table class="datatable table table-striped table-bordered table-vcenter">
                ...
                <tbody>
                    ...
                    @foreach ($form->controls as $num => $control)
                    <?php $change_status_action = $control->status ? 'deactivate' : 'activate'; ?>
                    <tr data-id="{{ $control->id }}" class="status-{{ $control->status }}">
                        <td class="text-center">{{ $num+1 }}</td>
                        <td class="status"><strong>{{ $control->name }}</strong></td>
                        <td class="text-center">
                            <?php
                             $buttons = array(
                                array(
                                    'url'   => action('Backend_FormControlsController@edit', $control->id),
                                    'type'  => 'edit',
                                    'tooltip'=> trans('backend.edit'),
                                    ),
                                 array(
                                    'url'   => action('Backend_FormControlsController@' . $change_status_action, $control->id),
                                    'type'  => $change_status_action,
                                    'tooltip'=> trans('backend.' . $change_status_action),
                                    'class' =>  'ajax-' . $change_status_action,
                                    ),
                                array(
                                    'url'   => action('Backend_FormControlsController@delete', $control->id),
                                    'type'  => 'delete',
